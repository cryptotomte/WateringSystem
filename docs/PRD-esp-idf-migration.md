...
- [ ] FR4: Två pumpar via GPIO/MOSFET med max-körtider, fail-safe-stopp vid ogiltig sensordata i autoläge, manuellt override.
- [ ] FR5: Reservoarstyrning med två nivåsensorer; polaritet styrd av board-konfiguration. Polaritetsfakta (från fix-branch 2026-04-12, verifierad mot KiCad-schema): XKC-Y26 OUT är aktiv HÖG (vatten = HIGH). Rev1 (direkt via TXS0108E, icke-inverterande) ⇒ GPIO aktiv HÖG. Rev2 (via 2N7002-inverterare) ⇒ GPIO aktiv LÅG. OBS: Arduino-koden läser aktiv LÅG på rev1 — känd bugg; verifiera slutgiltig polaritet med mätning på bänkriggen i fas 1 och dokumentera i paritetschecklistan (korrekt beteende, inte bug-för-bug-paritet).
- [ ] FR6: INA226-läsning per pump på rev2 (spänning/ström/effekt), exponerad i API och loggning.
