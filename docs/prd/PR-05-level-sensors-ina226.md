...
the FR5 bench verification that settles the rev1 polarity question) and an INA226
driver delivering raw voltage/current/power per pump on rev2.
...
- `Ina226Sensor` component on the shared `i2c_master` bus (from PR-03): one device per
  pump, distinct A0/A1-strapped addresses (must not collide with each other or BME280
  at 0x76/0x77); high-side shunts on the 12 V rail per rev2 BOM.
- Configure shunt calibration from Kconfig (shunt value per BOM), expose bus voltage,
  current, power per pump as plain readings for logging/API.
