 ...
 * @brief Implementation of SP3485ModbusClient class
...
bool SP3485ModbusClient::initialize()
{
    ...
    // Configure serial port for Modbus RTU (default: 9600 baud, 8 data bits, no parity, 1 stop bit)
                                            ...
                                            uint16_t count, uint16_t* buffer)
{
    ...
    // Validate parameters
    if (count > 125) { // Modbus spec limit
