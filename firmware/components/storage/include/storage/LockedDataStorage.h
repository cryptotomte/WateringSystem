 ...
 * A read-modify-write sequence spanning two calls (e.g. getSensorReadings
 * then storeSensorReading) is NOT protected against an interleaving writer
 * between the two — another task may append in between. Such sequences need
 * higher-level coordination (a caller-held lock or single-owner task).
 *
