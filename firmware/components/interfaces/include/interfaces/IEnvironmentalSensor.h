 ...
 *   2  read failed — bus/communication error during a data read, or the
...
public:
     ...
     * Returns false with error 1 when no BME280 is found. Idempotent, and
     * lazy-capable: read()/isAvailable() attempt initialization themselves
     * when it has not happened yet — calling this first is recommended,
     * not required (parity).
     */
     ...
     * getters are refreshed, or the call fails (error 1 on lost/absent
     * sensor, error 2 on bus error or NaN) and the last-good values remain
     * untouched. A bus error marks the driver uninitialized, so the next
     * call re-probes both addresses (recovery, FR-004). Exactly one bus
     * attempt, no retry — recovery comes from the caller's poll cadence.
     *
